...
def show_lounge(request):
  if request.method == 'POST':
    ...
    lounge_photos = Photo.objects.filter(lounge = True).order_by('upload_time')
...
def show_my_lounge(request):
  if request.method == 'POST':
    ...
    my_lounge_photos = Photo.objects.filter(user = user_obj,lounge = True).order_by('upload_time').values("img")
